...
Ainda, a função prova(), devolve por referência os nomes das variedades ( respostas[10][30]) que podem
ser cultivadas cujas condições são satisfeitas com os valores informados, além da quantidade de variedades (
*quantidade).*/

#include <stdio.h>
#include <string.h>
void prova(char variedades[10][30],
int temp_min[10], int temp_max[10],
int prec_min[10], int prec_max[10],
int alti_min[10], int alti_max[10],
int valor_pergunta1, int valor_pergunta2,
...
int main() {
    ...
    for (i = 0; i < 10; i++)
        scanf("%s %d %d %d %d %d %d", variedade[i],
        &t_min[i], &t_max[i],
        &p_min[i], &p_max[i],
        &a_min[i], &a_max[i]);
...
char respostas[10][30], int *quantidade){
    ...
    *quantidade=0;
    for (i = 0; i < 10; i++)
        if((valor_pergunta1<=prec_max[i] && valor_pergunta1>=prec_min[i]) && (valor_pergunta2<=alti_max[i] && valor_pergunta2>=alti_min[i])){
