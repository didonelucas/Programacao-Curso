...
void calcula(int horas_de_frio[5][5], int horas_de_frio_mes_media[5], float perc_horas_de_frio_mes[5], float *perc_total_horas_de_frio){
	int m, s, somaMes, somaTotal=0;
	for(m=0;m<5;m++){
		somaMes=0;
		for (s = 0; s < 5; s++){
			somaMes=somaMes + horas_de_frio[m][s];
		}
		somaTotal=somaTotal+somaMes;
		perc_horas_de_frio_mes[m] = (somaMes*100.0)/horas_de_frio_mes_media[m];
	}
	*perc_total_horas_de_frio = (somaTotal*100.0)/496;
}
