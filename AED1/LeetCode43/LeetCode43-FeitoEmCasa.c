...
char* multiply(char* num1, char* num2) {
    int tNum1 = strlen(num1);
    int tNum2 = strlen(num2);
    int tMaxResultado = tNum1 + tNum2; 
    
    int* aux = (int*)calloc(tMaxResultado, sizeof(int));
    int i, j;

    for (i = tNum1 - 1; i >= 0; i--) {
        for (j = tNum2 - 1; j >= 0; j--) {
            int multiplicacao = (num1[i] - '0') * (num2[j] - '0');
            
            int p1 = i + j;       //Casa das dezenas 
            int p2 = i + j + 1;   //Casa das unidades
            
            int soma = multiplicacao + aux[p2];
    ...
    j = 0;
    while (j < tMaxResultado && aux[j] == 0) {
        j++;
    }
    char* res = (char*)malloc((tMaxResultado - j + 1) * sizeof(char));
    int k = 0;
    for (i = j; i < tMaxResultado; i++) {
        res[k] = aux[i] + '0';
        k++;
    }
    res[k] = '\0'; 
